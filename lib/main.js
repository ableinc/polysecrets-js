...
class PolysecretsClass {
  ...
  __config__(obj) {
    ...
    'secret': 'rAnd0m_s3cr3t', 'verbose': false, 'persistence': {'host': 'localhost', 'port': 27017, 'db': 'polysecrets', 'collection': 'secrets'}}
      ...
      try {
        ...
        if (obj[key] && typeof obj[key] !== 'undefined') {
          if (typeof obj[key] !== typeof _defaults[key]) {
            console.log(`${obj[key]} has invalid type. You should have ${typeof _defaults[key]},
            but you have ${typeof _defaults[key]}.`)
            process.exit()
          }
        } else {
    ...
    if (this.config.automated) {
      const fetchSecrets = () => {
        this.__secret_generator()
          .then((final_secret) => {
            process.env.secret = final_secret

...
class PolysecretsClass {
  ...
  __config__(obj) {
    let _defaults = {'automated': false, 'interval': 30, 'length': 10, 'uuid': true, 'mixcase': false,
    'secret': 'rAnd0m_s3cr3+', 'verbose': false, 'persistence': {'host': 'localhost', 'port': 27017, 'db': 'polysecrets', 'collection': 'secrets'}}
    ...
    for (let key of Object.keys(_defaults)) {
      try {
        ...
        if (obj[key] && typeof obj[key] !== 'undefined') {
          if (typeof obj[key] !== typeof _defaults[key]) {
            console.log(`${obj[key]} has invalid type. You should have ${typeof _defaults[key]}, but you have ${typeof _defaults[key]}.`)
            process.exit()
          }
        } else {
          obj[last_key] = _defaults[last_key]
        }
        if (Object.keys(obj.persistence).length > 0) {
          for (let _key of Object.keys(_defaults.persistence)) {
            if (obj.persistence[_key] && typeof obj.persistence[_key] !== typeof _defaults.persistence[_key]) {
              console.log(`${obj[key]} has invalid type. You should have ${typeof _defaults[key]}, but you have ${typeof _defaults[key]}.`)
              return bad
            }
          }
  ...
  async __secret_generator() {
    ...
    return this.config.persistence !== false ? this.__persistence_loop(_final_secret) : _final_secret
  }
...
class Polysecrets {
  ...
  execute () {
    this.polysecrets = new PolysecretsClass(this.config)
